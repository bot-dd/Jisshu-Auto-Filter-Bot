...
async def send_movie_update(bot, file_name, files):
    try:
        ...
        for quality in sorted_qualities:
            ...
            tag_quality = f"#{quality.replace(' ', '')}"
            quality_links.append(QUALITY_CAPTION.format(tag_quality, " | ".join(file_links)))
        quality_text = "\n".join(quality_links)
        ...
        btn = [[
            ...
            InlineKeyboardButton("💥 Movie Search Group 🎬", url="https://t.me/Movies_Rm")
        ]]
        reply_markup = InlineKeyboardMarkup(btn)
...
def format_file_size(size_bytes):
    for unit in ['B', 'KB', 'MB', 'GB', 'TB']:
        ...
        size_bytes /= 1024
    return f"{size_bytes:.2f} PB"@Client.on_message(filters.chat(CHANNELS) & media_filter)
async def media(bot, message):
    bot_id = bot.me.id
    media = getattr(message, message.media.value, None)
    if media.mime_type in ['video/mp4', 'video/x-matroska','document/mp4']: 
