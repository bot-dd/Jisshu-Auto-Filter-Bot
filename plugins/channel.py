...
async def queue_movie_file(bot, media):
    try:
        ...
        language = (
            " ".join([f"#{lang.strip().replace(' ', '')}" for lang in CAPTION_LANGUAGES if lang.lower() in caption.lower()])
            or "#Unknown"
        )
...
async def send_movie_update(bot, file_name, files):
    try:
        ...
        kind = (
            "#" + imdb_data.get("kind", "Movie").strip().replace(" ", "")
            if imdb_data else "#Movie"
        )
        ...
        language = (
            " ".join(sorted(set(languages))) or "#Unknown"
        )
...
async def get_qualities(text):
    qualities = [
        "480p", "720p", "720p HEVC", "1080p", "1080p HEVC", "2160p", "HDRip", "WEB-DL",
        "HDTS", "HDTC", "HDCAM", "CAMRip", "DVDscr", "DVDRip", "Predvd"
    ]
    found_qualities = [q for q in qualities if q.lower() in text.lower()]
    return " ".join([f"#{q.replace(' ', '')}" for q in found_qualities]) or "#HDRip"
