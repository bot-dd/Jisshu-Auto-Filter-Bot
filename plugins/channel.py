...
async def send_movie_update(bot, file_name, files):
    try:
        ...
        language = ", ".join(f"#{lang.strip()}" for lang in sorted(languages)) or "#NotIdea"
        ...
        for quality in sorted_qualities:
            ...
            quality_links.append(
                QUALITY_CAPTION.format(f"#{quality}", " | ".join(file_links))
            )

        quality_text = "\n".join(quality_links)
        image_url = poster or "https://te.legra.ph/file/88d845b4f8a024a71465d.jpg"
        full_caption = UPDATE_CAPTION.format(title, language, f"#{kind}", quality_text)
